...
// Shared new-player phase-card layout: a header (phase name + time badge),
// one prescriptive tip, and availability pills. Each guide topic supplies
// its own PhaseCard[] and pill color map from its own lib/[topic] data module.
export function PhaseTimeline({ cards, pillColors }: PhaseTimelineProps) {
  return (
    <div>
      {cards.map((card) => (
            ...
            {card.available.map((label) => (
              <span
                ...
                className={`mt-2 mr-1 rounded-full border px-2 py-0.5 text-xs ${
                  pillColors[label] ?? "border-zinc-600 text-zinc-400"
                }`}
              >
                {label}
              </span>
